...
class Warp:
    ...
    def __init__(self, reverse: float = 0.0, speed: float = 1.0,
        ...
        self.speed_jitter = float(speed_jitter)  # per-grain random speed spread
    ...
    def _render(self, seg: Segment, ctx: Context) -> Segment | None:
        ...
        speed = self.speed * (1 + ctx.rng.uniform(-self.speed_jitter, self.speed_jitter))
        speed = max(0.1, speed)
        ...
        path = ctx.scratch_dir / f"warp_{uuid.uuid4().hex[:8]}.wav"
